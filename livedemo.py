...
square_side_length = 15 # millimeters
...
gray_color_image = cv.cvtColor(gray_image, cv.COLOR_GRAY2BGR)
image_height, image_width, channels = gray_color_image.shape
cv.resizeWindow(window_name, image_height, image_width)
cv.imshow(window_name, gray_color_image)
start_image = gray_image
start_color_image = gray_color_image
...
if ret == True:
    ...
    for r in range(board_rows):
        for c in range(board_cols):
            ...
            if c < board_cols - 1:
                ...
                cv.circle(gray_color_image, center1, 5, (255, 0, 0), 3)
                cv.line(gray_color_image, center1, center2, (255, 0, 0), 3)
                cv.circle(gray_color_image, center2, 5, (255, 0, 0), 3)
            ...
            if r < board_rows - 1:
                ...
                cv.circle(gray_color_image, center1, 5, (0, 0, 255), 3)
                cv.line(gray_color_image, center1, center2, (0, 0, 255), 3)
                cv.circle(gray_color_image, center2, 5, (0, 0, 255), 3)
    ...
    gray_color_image = start_color_image
    ...
    for c1 in range(board_cols):
        ...
        for r2 in range(board_rows):
            ...
            cv.circle(gray_color_image, center1, 5, (0, 255, 0), 3)
            cv.line(gray_color_image, center1, center2, (0, 255, 0), 3)
            cv.circle(gray_color_image, center2, 5, (0, 255, 0), 3)
    ...
    # this is the commented out section below that had to be moved up
    ...
    # because of the weird way that matplot wants information formatted at any given time...
    # we have to flip the y location array backwards... JUST for the px contours...
    # and then... flip it BACKWARDS AGAIN... after... don't @ me please... 
    # going to have to pay some extra attention to how all this information is utilized in the refactor
    # so that i don't asplode into a billion tiny pieces 
    y_px_array = np.flip(y_px_array)
    
    err_levels_px = np.arange(-10, 12, 1)
    err_levels_mm = np.arange(-5, 5.5, 0.5)
    ...
    CS = ax2.contourf(x_px_array, y_px_array, e_px_arranged, levels = 25, cmap = 'inferno')
#    CS2 = ax2.contour(CS, levels = CS.levels[::2], colors='b')
#    ax2.set_title('$ \epsilon $ in Calculated Real Distance from Bottom')
    ax2.set_xlabel('x location (pixels)')
    ax2.set_ylabel('y location (pixels)') # not sure what to label these
    
    xlim_2 = [x * 1.02 for x in ax2.get_xlim()]
    ylim_2 = [y * 1.02 for y in ax2.get_ylim()]
    
    ax2.set_xlim(xlim_2)
    ax2.set_ylim(ylim_2)
    
    cbar = contourfig.colorbar(CS)
    cbar.ax.set_ylabel('$ \epsilon $ (pixels)')
#   cbar.add_lines(CS2)
...
#    yax2.set_title('$ \epsilon $ in Calculated Y Real Distance from Bottom')
 ...
 #   ydax2.set_title('$ \epsilon $ in Calculated Y Real Distance from Bottom')
    ...
    """ moved this bit up, commented out for posterity,
    x_px_array = np.zeros((board_rows,board_cols))
    y_px_array = np.zeros((board_rows,board_cols))
...
#    ocbar.add_lines(overlay_contour2)
    ...
    plt.show()
    """ # trying to modify the above snippet to show coordinates in mm...
    ...
    overlayfig, ax3 = plt.subplots(layout = 'constrained')     
    ax3.imshow(gray_image, cmap = 'gray')
    ax3.set_title('Look Mom,\nI Got Calibrated At The\n 2025 MNSU Undergraduate\nResearch Symposium!', fontweight = 'bold', fontsize = 'x-large', color='magenta')
    overlay_contour = ax3.contourf(x_px_array, y_px_array, e_arranged, levels = 25, cmap = 'viridis', alpha = 0.7)   
    
    # funny axis conversion, big ups to chat gpt
    # had to break out a whole different part of matplot for this one
    
    # something is happening with the first figure whenever the posing is especially not nice
    # but not with the follow up figures?
    
    # the error mysteriously dissapeared after restarting a few times...
    # it is a mystery 🤷🏻‍♀
    
    def px_to_mm(x, _):
        return f"{x * mm_pixel_ratio:.2f}"
    
    ax3.xaxis.set_major_formatter(ticker.FuncFormatter(px_to_mm))
    ax3.yaxis.set_major_formatter(ticker.FuncFormatter(px_to_mm))    
    
  #  overlay_contour2 = ax3.contour(overlay_contour, levels = overlay_contour.levels[::2], colors='b')
    ax3.set_xlabel('x location (mm)')
    ax3.set_ylabel('y location (mm)') # not sure what to label these
    ocbar = overlayfig.colorbar(overlay_contour)
    ocbar.ax.set_ylabel('$ \epsilon $ (mm)')
#    ocbar.add_lines(overlay_contour2)
